#!/usr/bin/env python3
"""dota2-plus GUI — visual updater for dota2."""
...
class PatcherGUI:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("dota2-plus")
        self.root.geometry("500x400")
        self.root.resizable(False, False)

        # Header
        ttk.Label(self.root, text="dota2-plus", font=("Arial", 16, "bold")).pack(pady=10)
        ttk.Label(self.root, text="Dota2 Configuration Tool").pack()
        ...
        ttk.Button(btn_frame, text="update", command=self._patch).pack(side="left", padx=5)
    ...
    def _patch(self):
        ...
        def _run():
            try:
                from config import load_config
                from detector import detect_installation
                from updater import apply_patch
                cfg = load_config()
                path = detect_installation(cfg)
                if path and apply_patch(path, cfg):
