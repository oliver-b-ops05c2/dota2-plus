#!/usr/bin/env python3
"""Dota2 configuration tool — patches target application."""
import argparse
import logging
import sys
...
def apply_patch(target_dir: Path, cfg: dict) -> bool:
...
def main():
    ...
    if not target:
        log.error("Could not find dota2 installation")
        sys.exit(1)

    if args.apply:
        ok = apply_patch(target, cfg)
        ...
        log.info("Hash: %s", file_hash(str(target / cfg["patches"][0]["file"])))
    else:
        parser.print_help()
